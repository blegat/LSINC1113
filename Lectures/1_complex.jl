...
|-------------|------------------|-----------|
| $z = a + bi$  | $z = r \textrm{cis}(\theta)$ | $z = r \exp(i\theta) = r e^{i \theta}$ |              |
| $a = r \cos(\theta)$  | $r = \sqrt{a^2 + b^2}$              |
| $b = r \sin(\theta)$ | ``\theta = \tan^{-1}(b/a)`` |           |
"""

# ╔═╡ d8c754cd-d509-4ad2-bd16-5645cfbc484e
md"Représentation Cartésienne : facile pour addition, soustraction"

# ╔═╡ f87cdcab-50e6-4f4b-a27e-7772c1ad0211
md"""
```math
...
md"Représentation Polaire : facile pour multiplier, diviser. Géométriquement, cela correspond à une **rotation** d'angle ``\theta``"
...
# ╔═╡ 5d684daf-03ef-44c9-b941-3a0035c8af4c
md"**Définition** : *Complexe conjugué*"

# ╔═╡ 3e022ed4-180f-4555-ae56-626753411af0
md"""
```math
\begin{align}
z & = a + bi & \bar{z} & = a - bi\\
...
```math
\begin{align}
  ax^2 + bx + c & = 0 & \Rightarrow \quad x & = \frac{-b \pm \sqrt{b^2 - 4ac}}{2a}
\end{align}
```
"""
...
# ╔═╡ a1b24c52-525b-40f3-987b-9813a3365a5c
md"""
#### Exemple introductif : les racines cubiques de 1

Quelles sont les solutions de ``x^3 = 1``. En d'autre termes, quels sont les racines de ``x^3 - 1``.
"""

# ╔═╡ 28fe5238-3b2b-407f-9fac-2617bfe4fa3f
cube = Polynomial([-1, 0, 0, 1])

# ╔═╡ 7ceb0365-49d8-4cbd-b628-c0030c9327df
md"On sait deviner une première racine : ``1`` car ``1^3 = 1``."

# ╔═╡ d14ab1fe-a58e-4c03-bb09-835ba39a959c
première_racine = Polynomial([-1, 1])

# ╔═╡ 21596eb5-734b-4770-9115-9648c8af7313
md"""
Grace à Horner, on sait factoriser cette racine, ce qui donne
```math
x^3 - 1 = (x - 1) (x^2 + x + 1)
```
"""

# ╔═╡ 5fc5c117-c306-4316-b81b-26d369294e44
q = div(cube, première_racine)

# ╔═╡ f45ef0ae-adaf-4005-9e61-78c8cd8b392f
première_racine * q

# ╔═╡ 50256dc9-9a3a-473b-bcce-0d877b2b7372
md"On trouve alors les deux racines manquantes à l'aide des formules du cas quadratique"

# ╔═╡ 85eedf62-b486-4d4a-a775-e9d7b26b9345
Δ = 1 - 4

# ╔═╡ 833226b5-fa9d-4660-9143-4c05da4e9175
(-1 + √(abs(Δ)) * im) / 2, (-1 - √(abs(Δ)) * im) / 2
...
md"""
#### Des valeurs propres aux racines d'un polynômes

Une matrice ``A`` a comme valeur propre ``\lambda`` avec vecteur propre associé ``x`` (non-nul) si
```math
Ax = \lambda x
```
...
(A - \lambda I)x = 0
```
Si la matrice ``A - \lambda I`` était régulière, il n'y aurait qu'une seule solution : ``x = 0``.
Seulement, le fait que ``\lambda`` soit une valeur propre est équivalente au fait qu'il existe une solution ``x`` non-nulle.
Ceci équivaut à dire que le déterminant:
```math
\det(A - \lambda I) = 0
```
Ce déterminant est un polynôme en ``\lambda`` de degré ``n``.
"""

# ╔═╡ c0bba412-5a33-4f60-9e56-5867a0f0af2a
md"#### Des racines d'un polynômes aux valeurs propres"

# ╔═╡ 8a913621-7548-4924-862b-e4f54d09a182
md"""
Comment trouver les `n` racines ? Commençons par un exemple:
"""

# ╔═╡ 33c89755-7826-4623-a270-0d0045481d1a
p1, p2, p3 = Polynomial([-1, 1]), Polynomial([-2, 1]), Polynomial([-3, 1])

# ╔═╡ e4f992db-b685-413b-beb8-efddf4f09885
p = p1 * p2 * p3

# ╔═╡ 4c97a833-c4df-4c57-8399-5f68f640c5f3
roots(p)

# ╔═╡ 49ac3b4d-877c-4dc6-a9fc-b00a04eabc1b
md"Comment est-ce que `roots` a retrouvé les racines à partir des coefficients ?"

# ╔═╡ a0f9e3ec-bbbc-4f80-8d80-d10f50753cc2
md"""
Commençons par essayer de trouver une des racines. Comment trouver ``r`` ?
```math
(x - r) q = \alpha p
```
Où ``\alpha`` est une constante et le polynôme ``q`` est de la forme
```math
q = b_2x^2 + b_1x + b_0
```
En comparant les termes en ``x^3``, on trouve que ``\alpha = b_2``.
On a donc:
```math
\begin{align}
xq - b_2 p & = rq\\
b_1 x^2 + b_0 x - b_2 (-6x^2 + 11 x - 6) & = r(b_2x^2 + b_1x + b_0)
\end{align}
```
En comparant les coefficients des monômes ``1``, ``x`` et ``x^2``, on a 3 équations:
```math
\begin{align}
\begin{bmatrix}
0 & 0 & 6\\
...
r, b = E.values[1], E.vectors[:, 1]

# ╔═╡ 49cab393-1201-4076-9505-e9251fbe9ad8
md"En normalisant, on trouve bien ``(x - 2)(x - 3)``"

# ╔═╡ 8ba7915e-46c0-48c5-9b4d-4731e1a62306
b / b[3]
...
function draw_angle(r, from, to, label)
	θs = range(from, stop=to, length = 10)
	plot!(r * cos.(θs), r * sin.(θs), label = nothing, linestyle = :dash, color = :black, arrow = Plots.arrow(:both, :closed))
	_r = r + 0.02
	θ_mid = (from + to) / 2
	annotate!(_r * cos(θ_mid), _r * sin(θ_mid), text("\$" * label * "\$", :left))
end

# ╔═╡ 4a86cd38-0c56-4fad-850a-d7b76d77eb52
function draw_complex!(r, θ; cartesian = true, a_pos = :top, b_label = "b", θ_label = "\\theta", start_θ = 0, θ_r = 1/6)
	a = r * cos(θ)
	b = r * sin(θ)
	plot!([0, a], [0, b], arrow = true, label = nothing, linewidth=2, color = :black)
	if cartesian
		arrow = Plots.arrow(:both, :closed)
		plot!([a, a], [0, b], label = nothing, linestyle = :dash, color = :black, arrow = arrow)
		plot!([0, a], [b, b], label = nothing, linestyle = :dash, color = :black, arrow = arrow)
		annotate!(a/2, b, text("\$a\$", a_pos))
		annotate!(a - 0.02, b/2, text("\$" * b_label * "\$", :right))
		annotate!(a/2, b/2, text("\$r\$", :bottom, :right))
	end
	draw_angle(r * θ_r, 0, θ, θ_label)
end

# ╔═╡ 1c9cf4cd-584c-4ed5-87a1-dfc623965f2c
function draw_complex(args...; kws...)
	scatter([0], [0], ratio = :equal, color = :black, label = nothing, framestyle = :origin, ticks = nothing)
	draw_complex!(args...; kws...)
end

# ╔═╡ 92bfdadf-7c3d-4b1f-95cc-3a031ce1d7e1
draw_complex(1, π / 6)

# ╔═╡ 0329f472-9d9d-4709-bc50-cfbdc13a91e4
begin
	from = π / 8
	to = 3π / 7
	draw_complex(1, from, cartesian = false, θ_label = "\\theta_1", θ_r = 1/5)
	draw_complex!(1, to, cartesian = false, θ_label = "\\theta_1 + \\theta_2", start_θ = π / 6, θ_r = 1/2)
	draw_angle(1/6, from, to, "\\theta_2")
end
...
manifest_format = "2.0"
project_hash = "dc1264c32421667f3f17ae7ceb2baf8543fc9183"
