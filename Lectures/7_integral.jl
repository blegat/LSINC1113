...
# ╔═╡ 86374740-9a8e-11ef-3ad2-950a7b092eb9
md"# Intégrale bivariée"

# ╔═╡ 8e2594c3-1c6e-4085-b7c3-5682317ae547
md"""
Nous avons vu comment les dérivées univariées étaient généralisées au dérivées multivariées via les dérivées partielles.
Nous avons ensuite implémenté le calcul de dérivées automatique.
Pour les intégrales, nous allons nous limiter au cas bivarié car la méthode est essentiellement graphique et elle devient donc plus compliquée à plus de 2 variables.

## Exemple introductif

Considérons le calcul de l'intégrale
```math
\int_{x \in [0, 1], y \in [0, 1]} xy \,\text{d}x\,\text{d}y
```
Ceci correspond à calculer le volume sous la surface suivante dans le carré ``[0, 1] \times [0, 1]``.
"""

# ╔═╡ 9bfd2996-3213-40f2-bf70-9b7b4d2ee0c4
let
	plotly()
...
# ╔═╡ 38c1498b-426c-408a-b779-229179b72ac8
md"""
Pour le calcul de dérivées multivarié, on s'est ramené au cas univarié en considérant les dérivées partielles. Dans une dérivée partielle, une seule des variable était considérée comme "variable" et les autres étaient considérées comme "constantes".
Pour les intégrales, on procéder de façon similaire.
On va se ramener à des intégrales univariées où une seul des variables sera intégrée et les autres seront considérées comme constantes.
En prenant l'interprétation géométrique du calcul de volume sous une surface,
on voit qu'intégrer d'abord en ``x`` puis en ``y`` ou en ``y`` puis en ``x`` devrait donner le même résultat. Intégrons par exemple d'abord en ``x``. Comme ``y`` est constant, on peut le sortir de l'intégrale en ``x``:
```math
\begin{align}
\int_{y = 0}^1 \int_{x = 0}^1 xy \,\text{d}x\,\text{d}y
& =
\int_{y = 0}^1 y\int_{x = 0}^1 x \,\text{d}x\,\text{d}y\\
& =
\int_{y = 0}^1 y [x^2/2]_{x=0}^1 \,\text{d}y\\
& =
\frac{1}{2} \int_{y = 0}^1 y \,\text{d}y\\
& =
\frac{1}{2} [y^2/2]_{y=0}^1\\
& =
\frac{1}{4}
\end{align}
```
...
# ╔═╡ 165227b1-e06b-4ecd-b875-7fa8bca4446a
md"""
## Changer l'ordre pour se simplifier la vie

Bien que le choix d'ordre de l'intégrale ne change pas le résultat, un ordre peut être plus facile à intégrer qu'un autre!
Considérons par exemple l'intégrale suivante:
```math
\int_{y = 0}^1 \int_{x = 0}^1 x\sin(xy) \,\text{d}x\,\text{d}y
```
Ceci correspond à calculer le volume sous la surface suivante dans le carré ``[0, 1] \times [0, 1]``.
"""

# ╔═╡ 5bd3c058-0ce8-4f23-81ff-b78904c8b736
let
	plotly()
	x = range(0, stop=1, length=10)
	surface(x, x, (x, y) -> x * sin(x * y))
end

# ╔═╡ 4b71b849-096c-4509-a342-ec7a8859a0b5
md"""
Trouver la primitive de ``x\cos(x y)`` par rapport à ``x`` n'est pas évident 🤔
Essayons d'inverser l'ordre d'intégration:
```math
\int_{x = 0}^1 \int_{y = 0}^1 x\sin(xy) \,\text{d}y\,\text{d}x
```
On peut sortir ``x`` parce qu'il ne dépend pas de ``y``:
```math
\int_{x = 0}^1 x \int_{y = 0}^1 \sin(xy) \,\text{d}y\,\text{d}x
```
On doit maintenant trouver la primitive de ``\cos(xy)`` par rapport à ``y``, c'est plus facile 😌
```math
\begin{align}
\int_{x = 0}^1 x [-\cos(xy)/x]_{y=0}^1 \,\text{d}x
& =
\int_{x = 0}^1 x (-\cos(x)/x + 1/x) \,\text{d}x\\
& =
\int_{x = 0}^1 1 - \cos(x) \,\text{d}x\\
& =
[x - \sin(x)]_{x=0}^1\\
& =
1 - \sin(1)
...
# ╔═╡ eab3f90d-3742-44fc-a0a7-43f95b03e558
md"""
## Intégrer avec bornes

Supposons maintenant qu'on doive intégrer avec des bornes en ``y`` qui dépendent de ``x``.
```math
\begin{align}
\int_{y = 0}^{1/2} \int_{x = 1 / (1 - y)}^{2} x\sin(xy) \,\text{d}x\,\text{d}y
\end{align}
```
Le domaine en ``(x, y)`` a maintenant la forme ci-dessous et il faut calculer le volume entre cette forme et la surface définie par la fonction ``x \sin(xy)``.
"""

# ╔═╡ c5b7ef94-10f5-4113-90dc-bc436f262758
md"""
Le principe reste le même que pour les intégrales à bornes constantes, la seule différence c'est qu'on évalue la primitive à des valeurs qui ne sont pas constantes.
Seulement, on se retrouve à nouveau à devoir calculer la primitive de ``x \sin(x)`` qui n'est pas évidente.
Comme précédemment, on va profiter de cette occasion pour illustrer le changement d'ordre dans les intégrales.

Mais comment faire ici avec les bornes de ``y`` qui dépendent de ``x`` ?
En regardant le graphe ci-dessus, on voit que les valeurs de ``x`` vont de ``1`` à ``2``.
Pour un certain ``x``, les valeurs de ``y`` vont de ``0`` jusqu'à la courbe ``x(y) = 1 / (1 - y)``.
Comme on a la valeur de ``x`` en fonction de ``y`` pour cette courbe et non l'inverse, on va devor l'inverser:
```math
\begin{align}
  x & = \frac{1}{1 - y}\\
  \frac{1}{x} & = 1 - y\\
...
\end{align}
```
On peut donc réécrire l'intégrale comme suit:
```math
\begin{align}
\int_{x = 1}^2 \int_{y = 0}^{1 - 1/x} x\sin(xy) \,\text{d}y\,\text{d}x
\end{align}
```
"""
...
md"""
On peut maintenant dérouler le calcul de l'intégration:
```math
\begin{align}
\int_{x = 1}^2 \int_{y = 0}^{1 - 1/x} x\sin(xy) \,\text{d}y\,\text{d}x
& =
\int_{x = 1}^2 x [-\cos(xy)/x]_{y=0}^{1 - 1/x} \,\text{d}x\\
& =
\int_{x = 1}^2 x (1 / x - \cos(x - 1)/x) \,\text{d}x\\
& =
\int_{x = 1}^2 1 - \cos(x - 1) \,\text{d}x\\
& =
[x - \sin(x - 1)]_{x=1}^2\\
& =
2 - \sin(1) - 1 + \sin(0)\\
